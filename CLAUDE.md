...
## 스킬 규칙 (2026-08-23 인벤토리 + 8-24 사용규칙 — 단일 절 통합)
> 스킬 규칙은 **이 절 하나뿐이다.** 같은 항목이 두 절에 나뉘어 표기가 갈렸던 사고(humanize-korean,
> 2026-08-25 통합)가 재발하지 않도록, 스킬 관련 규칙 추가·수정은 반드시 이 절에서만 한다.
...
- **`ui-copy` = 사용자 노출 문구 상시 적용**(2026-09-08 오너 지시로 mri-student-app에서 복사). UI·오류·빈 화면·버튼·알림·
  봇 응답·DM·웹 카피를 쓰거나 고칠 때 `.claude/skills/ui-copy/SKILL.md`를 따른다. 코드 주석·로그·오류 코드 식별자
  (`slot_taken` 등)·개발자 문서는 대상이 아니다. **전수 교체는 하지 않는다**(오너 지시) — 만지는 화면부터 자연스럽게 적용한다.
  ⚠️ **운영진 대상 봇 메시지의 기존 반말체는 유지한다.** 스킬의 "~요"체는 **수강생·외부 사용자에게 가는 문구** 기준이고,
  트레이너·오너용 명령 응답까지 한 번에 바꾸면 전수 교체가 된다.
- **`mri-canonical-operations` = 상시 적용.** DDL·데이터 변경·PR 전 과정. "예상 밖 파일이 바뀌면 중단"이
  여기 있고, 이 규칙이 #132·#136 유형을 사전 차단했다. 우선순위 최상위.
- **`humanize-korean` = 미설치 · 설치 시 적용** (정본 = 8-24 판정. 구 8-23 표기 "PR 본문·회신에 적용"은 폐기).
  설치 보류(2026-08-24) — 외부 발송 문안의 최종 작성은 관제탑 소관이고 세션 산출물(SQL·보고·PR)은 전부
  사용 금지 범위라 설치해도 쓸 곳이 없다. `/plugin` 미지원 환경이라 세션 안 설치도 불가(실측).
...
### 저장소 스킬 5종 (`.claude/skills/` — 이 repo와 함께 배포)
| 스킬 | 읽기 범위 | 트리거 |
|---|---|---|
...
| `ui-copy` | 사용자에게 보이는 한국어 문구 전부(사이트 카피·봇 응답/DM·오류·빈 화면·버튼 라벨) — repo 파일 미변경, 톤 정본 | 문구를 **한 줄이라도** 쓰거나 고칠 때. **한국어 트리거 있음**. 정본은 mri-student-app이고 이 사본은 복사본이다 — 한쪽을 고치면 반대쪽도 고친다 |
