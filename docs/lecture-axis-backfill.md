...
> ⛔ **위임 실행 중단(2026-08-22) — 드라이런에서 발행 블록과 실DB가 어긋나 미실행 회신.**
> P-1 통과 후 착수 조건은 충족했으나, 실행 전 드라이런 실측(courses 8행 대조)에서 3건:
> ① **양형석(course 4)이 `active`·초급반**이라 D-2가 8회를 차감한다 — 방금 실행된 P-1
>   음수 행(4/10 레슨 전환)과 정면 모순. course 4 상태 정리(전환 종료) 또는 D-2 제외 판정 필요.
> ② **D-1 세션 8건 전부 label '초급반'**인데 8월 5건(8/5~17)은 시기상 허혜민·김해주
>   (**심화반** · 8/1~2 시작 · 8회 계약)의 수업일 개연성 — 이대로면 심화반 2명 참석 0,
>   초급반 권태완·양형석만 8회 차감. 「실수업 반 사실대로 기록」 판정과 충돌. 세션별 실제
>   반 라벨 확정 필요(강의일정 시트 대조).
> ③ 6~7월 3건(6/29·7/4·7/5)의 초급반 재적은 권태완뿐 — 참석자 목록 자체가 필요하다.
> 드라이런 D-2 예상: 권태완 8회·양형석 8회 참석, 나머지 6계약 전부 0. 사후검증(잔여
> 음수 0)은 이 상태를 **통과시켜 버리므로** 검증으로 못 잡는다 — 세션×반×참석자 확정 후
> 재발행이 정본이다. 위임 규칙 「수정이 필요하면 실행 말고 회신」에 따름.
...
#### P-1 직강 결제 원장 시드 — ✅ 위임 실행 완료 (2026-08-22 · 관제탑 승인 ⓐⓑⓒ)
