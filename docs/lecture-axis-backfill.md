...
| 신종근(56) | 심화반 | 3/18 | 254,000(3차) | 12회* | ✅ **Q-5 확정** — **254,000(3차)만 시드.** memo 「1·2차 원장 개시 이전 — 금액 불명, 오너 확인 대기」. *12회는 「직강 결제는 전부 구 체계 정규(12회/36회)」(관제탑 8/21)에서 온 전제, status='done'은 students 상태 정합 — 다르면 정정 요청 |
| 박성민(25) | 심화반 | 6/12 | 360,000 +미입금 90,000 | 12회 | **C-1** — `<반>`=심화반 확정 |

박성진(심화)·최창운(심화)은 반은 확정됐으나 **원장 결제 미확인** — VALUES 제외 유지.
...
select s.id, v.level, 'old', 180, 30000, v.units, v.started, v.status, 'sheet_import', 4, v.memo
  from (values
    ...
    ('신종근',   '심화반', 12, date '2026-03-18', 'done',
     '직강 백필(구체계) · 3차 254,000만 시드 · 1·2차 원장 개시 이전 — 금액 불명, 오너 확인 대기 · 12회=구체계 정규 전제 · Q-5 확정 8/21 · owner_sql')
...
-- 기대: INSERT 7 (박선우·길영패는 STEP A 등재가 선행이다 — 미등재면 join에서 빠져 행수가 준다)
```
