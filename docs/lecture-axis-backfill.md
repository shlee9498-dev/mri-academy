...
**원장 실측 8건 → 처리 분류** (v3 · `<반>` 매핑 + Q-3~5 판정 반영, 관제탑 8/21):

| 학생 | 반(확정) | 결제일 | 실계약 | 회차 | 처리 |
|---|---|---|---:|---|---|
| 박선우 | 심화반 | 4/10 | 360,000 | 12회 | ✅ VALUES (STEP A 등재 후) |
| 권태완(39) | 초급반 | 4/11 | 360,000 | 12회 | ✅ VALUES |
| 길영패 | 중급반 | 5/6 | 360,000 | 12회 | ✅ VALUES — 원장 「길영태」= 동일인(오타 확정) |
| 양정현 | 중급반 | 6/6 | 360,000 | 12회 | ✅ VALUES |
| 김준성 | 중급반 | 4/21 | 870,000 | 36회 | ✅ **Q-3 확정** — scheme='old'·36회·870,000·started 4/21. 신체계 역산 금지. 저장은 #137 패턴(단가 정가 30,000 · 실계약 870,000은 memo — `unit_price`가 integer라 비정수 단가 불가) |
| 양형석(43) | 초급반 | 4/3 | 360,000 | 12회 | ✅ **Q-4 확정** — courses 계약 **원본 12회/360,000** 그대로. 4/10 전환 −120,000은 **payments 음수 행 대응**(직강 결제 원장 시드 시 별도 — courses에서 회차를 깎지 않는다) |
| 신종근(56) | 심화반 | 3/18 | 254,000(3차) | **불명** | 🔴 **시드 보류 → 오너 확인 대기**(관제탑 Q-5 반려 8/21 — 12회 전제 근거 없음, 원장 비고는 「정규강의 추가분(3차)」뿐). 실측: `units_total`은 null 가능하나 **`unit_price`가 NOT NULL**이라 단가 추정 없이는 행 자체를 못 만든다 → 추정값 입력 금지 원칙으로 보류. `settlement-corrections.md` W-5. status `done` 유지 확정(8월 수업 명단에 없음 — 복구 안 함) |
| 박성민(25) | 심화반 | 6/12 | 360,000 +미입금 90,000 | 12회 | **C-1** — `<반>`=심화반 확정 |

박성진(심화)·최창운(심화)은 **「수업 실재·결제 미기록」 유형 확정**(관제탑 8/21 — 원장 전수 196행에 결제 0건, 일정표에는 심화반 실재. 박지훈과 같은 축) — VALUES 제외 유지, `settlement-corrections.md` W-6 오너 확인 대기.
반 매핑의 장익교·엄태현·허혜민·이희훈·김웅채는 이번 백필 VALUES 대상이 아니다
(기존 행 보유·조건부·결제 미대조).

**실행** — 반은 관제탑 8/21 매핑으로 채웠다. `session_minutes=180`은 기존 2행 관례의
잠정값이다(다르면 정정):
...
select s.id, v.level, 'old', 180, 30000, v.units, v.started, v.status, 'sheet_import', 4, v.memo
  from (values
    ('박선우',   '심화반', 12, date '2026-04-10', 'active',
     '직강 백필(구체계) · 원장 4/10 360,000 · owner_sql'),
    ('권태완',   '초급반', 12, date '2026-04-11', 'active',
     '직강 백필(구체계) · 원장 4/11 360,000 · owner_sql'),
    ('길영패',   '중급반', 12, date '2026-05-06', 'active',
     '직강 백필(구체계) · 원장 5/6 360,000 · 원장 표기 길영태 = 동일인(오타, 관제탑 8/21 확정) · owner_sql'),
    ('양정현',   '중급반', 12, date '2026-06-06', 'active',
     '직강 백필(구체계) · 원장 6/6 360,000 · owner_sql'),
    ('김준성',   '중급반', 36, date '2026-04-21', 'active',
     '직강 백필(구체계) · 실계약 870,000(정가 1,080,000 − 할인 210,000 · #137 패턴: 단가 정가 유지·차액 memo) · 원장 4/21 · Q-3 확정 8/21 · owner_sql'),
    ('양형석',   '초급반', 12, date '2026-04-03', 'active',
     '직강 백필(구체계) · 원장 4/3 360,000/12회 · 4/10 전환 −120,000은 payments 음수 행 대응(원장 시드 시 별도, 회차 불변) · Q-4 확정 8/21 · owner_sql')
  ) as v(name, level, units, started, status, memo)
...
select 25, '심화반', 'old', 180, 30000, 12, date '2026-06-12', date '2026-08-14', 'done',
       'sheet_import', 4,
...
> 🔄 **관제탑 재판정(2026-08-22) — 드라이런 중단 회신 3건에 대한 판정. 아래 블록은
> 전부 폐기 상태이며, 정본 소스 교체 후 재발행한다.**
> - **세션 정본 = 오너 보유 `MRI_주간_스케쥴.xlsx`의 수업_로그(506행).** 강의일정표는
>   일정·참석자 소스이지 차감 원장이 아니다 — 「강의일정표 8건으로 D-1」 지시 자체가 폐기됐다.
> - ① **양형석 course 4는 `active` 유지**(종료 판정 보류·D-2 제외도 하지 않는다).
>   정본: 강의 진행·완료 **7회 · 잔여 5회 · 잔액 150,000**. 거래내역 분석상 입금 620,000
>   vs 등록 360,000(차액 260,000 「결제 누락 가능성」)이 잡혀 있어 4/10 −120,000 레슨
>   전환은 별개 건일 개연성 — 실제 수업 여부는 수업_로그로 판정한다.
> - ② 오라벨 지적은 정확하나 교체 소스는 강의일정표가 아니라 **수업_로그**다. 오너가
>   DB이관 추출본(**DB이관_세션 · 미기입_8월**)을 제공하면 그것으로 **D-1부터 재발행**.
> - ③ 참석자도 같은 경로 — 수업_로그는 개인명 행·반 단위 행 혼재라 추출본에
>   **반단위여부(Y/N) 열**이 포함돼 온다.
> 선행: STEP C `started_on` 6행 정정 + 미이관 7명 STEP C-2(관제탑 별도 발행분 —
> started_on이 회차 차감·FIFO 기준이라 STEP D 재발행보다 먼저다).
...
#### P-1 직강 결제 원장 시드 — ✅ 종결 (관제탑 독립검증 합격 2026-08-22)

> **관제탑 독립검증 합격 — 직강 축 결제 시드 종결.** 9행/3,164,000 · kind=course
> 11행/3,870,000 · 미연결 2행 · 양형석 음수 rate 0 course_id=4 전부 일치 확인.

> **실행 완료(8/22)** — 관제탑 동결 해제·위임 승인(ⓐ 양형석 음수 행 course 연결
> ⓑ 미연결 2행 memo에 「courses 계약 행 확정 후 연결 예정(신종근 W-5 / 박성민 C-1)」
> 명시 — 시드 memo에 문구 추가 반영 ⓒ fee/net 생략) 후 아래 블록 그대로 실행.
> 사전검증: 동명 전원 1 · 연결 대상 7행(양형석 음수 행 → 원계약 course 4) · 기존재 0.
> **사후검증 3/3 통과(원시 출력은 실행 회신에 첨부)**: ① 9행 · 3,164,000
> ② kind='course' 전체 11행 ③ 미연결 = 박성민 6/12 360,000 · 신종근 3/18 254,000 뿐.
