...
## 7. FIFO 백필 (D구간) — ✅ **전체 종결 (2026-08-25 밤 · 관제탑 독립검증 합격)**

> 8/22 재산출 62행/352판 → fit 41(§7-3) → straddle 3 → 주현성 재귀속 2 → 김예지 7 →
> 정정 3(§7-6) → 신규 유입 2(§7-7) → 마지막 물결 14(§7-9) = **비시드 미귀속 0행**.
> 독립검증(관제탑 8/25 밤): 미귀속 0 · 귀속 94 · 등록별 21=9/30=5/31=10/116=3/117=−23 ·
> 김재성 157 · 장익교 56 — 전부 실측 일치 · **합격**. 이후 신규 유입은 §7-8 FIFO 승격
> (#269 배포)이 자동 귀속한다. 이하 각 절은 이력 보존.
...
### 7-6. W-2 정정 3건 — ✅ **위임 실행 완료 (2026-08-25 밤 · 관제탑 §7-7 묶음 지시)**

> **실행 결과**: INSERT 3 확인(검증 ① — 8/2:+10 · 9/2:−32 · 9/5:−38). 사후검증 전부
> 기대 일치: 장익교 총 **56**·잔여 **10** / 김재성 총 **157**·잔여 **18** / 담당별
> 15·41·99·58(아래 검산 그대로) / 미귀속 seed외 **14** = W-2 원행 11 + 정정 3행(본 절
> 발행문 「실행 직후 11→14 정상」 그대로 — 관제탑 지시문의 기대 「11」은 §7-7 단독
> 기대값과의 미합성이고 발행 정본 기대는 14다. 신규 2행은 §7-7로 귀속 종결).
> 이하 발행 원문 보존.
...
> ✅ **판정 확정(관제탑 8/25 밤): 안2 — 현행 유지.** DB 변경 0(담당 그대로 — 담당별
> 잔여 준구 −8/현태 +18은 straddle (b) 음수 허용 판례와 동형). **마지막 물결(미귀속
> 14행) 발행의 선행이 해소됐다** — 다음 라운드에서 FIFO 재산출·발행.
...
### 7-7. 신규 유입 미귀속 2행 — ✅ **위임 실행 완료 (2026-08-25 밤 · §7-6과 같은 라운드)**

> **실행 결과**: UPDATE 2 — #145→127 · #146→128 확인. 등록 127 잔여 16→**12** ·
> 128 잔여 17→**13** — 기대 일치. 신규 2행 귀속 종결. 이하 발행 원문 보존.
...
(→ 관제탑 8/25 밤 판정: **봇 v2로 미루지 않는다** — §7-8로 승격.)

### 7-8. `resolveEnrollmentId` FIFO 승격 — 설계 (2026-08-25 관제탑 지시 · 컷오버 전 배포)

> 관제탑 판정: 9/3 컷오버 후 DB가 정산 정본이 되면 미귀속이 곧 지급 누락이다. 백필로
> 사후 수습하는 구조를 컷오버까지 끌고 가지 않는다. **§7-2 승인 규칙의 코드화**이지
> 새 판정 로직이 아니다. **설계 발행 → 관제탑 검토 → 위임 구현 → 9/3 전 배포.**

#### 선택 규칙 (관제탑 확정 8/25)

1. **트레이너 일치 필수** — 세션 `trainer_id` = 등록 `trainer_id` (주현성 판례)
2. `status in ('active','paused')` 등록을 **`started_on` 오름차순**으로
3. **잔여 > 0 인 첫 등록**에 귀속 (잔여 = `games_total + bonus_games` − 기귀속 세션 합)
4. 전 등록이 잔여 ≤ 0(소진·초과)이면 **귀속하지 않고 null** + 오너 알림(현행 unattached 경로)

#### 구현 스케치 (server.js — `resolveEnrollmentId` 교체, ~25줄)

- 현행: `carry_games≠0 → null` / 등록 1건 → 그 id / 2건+ → null.
- 변경: carry_games 가드 **유지** → 등록 조회를 `trainer_id=eq` + `status=in.(active,paused)`
  + `order=started_on.asc`로 → 각 등록의 기귀속 합을 조회해 잔여 계산 → 첫 잔여>0 반환 →
  없으면 null(현행 unattached 오너 DM 경로 그대로 — 규칙 4).
- 시그니처 `resolveEnrollmentId(studentId, trainerId)` — 호출부(`dualWriteSessions`)가
  trainer_id를 이미 갖고 있어 변경 1곳.
- **자동 귀속은 안전한 부분집합만**: straddle의 (b) 선행 초과 허용은 백필 판정 전용으로
  남긴다 — 자동 경로는 잔여>0 등록에만 붙이고, 애매하면 null+알림으로 떨어져 위임 판정으로 간다.

#### 관제탑 검토 요청 1건 — 부분 초과의 해석

잔여 2 남은 등록이 첫 후보인데 세션이 4판이면? FK 1개라 쪼개기 불가(straddle 판례).
- **ⓐ(권고)** 잔여>0이므로 그 등록에 통째 귀속(초과 허용 — straddle (b) 판례와 동형,
  음수 잔여를 후속 등록이 흡수)
- ⓑ 초과가 나면 null + 알림(더 보수적 — 단 미귀속이 다시 쌓인다)

이 1건 외에는 §7-2 그대로라 추가 판정 불요.

> ✅ **검토 회신 수신(8/25 밤): ⓐ 통째 귀속 채택 → 위임 구현 완료.**
> `resolveEnrollmentId(studentId, trainerId)` 교체(트레이너 일치 필수 → started_on asc →
> 잔여>0 첫 등록에 통째 → 전 소진 null) + 호출부 1곳. npm check 통과.
> **머지·배포(=봇 재기동)로 실효** — 컷오버(9/3) 전 배포 요건 충족. (#269 머지·배포 8/25 밤)

### 7-9. 마지막 물결 — 미귀속 14행 FIFO 배정 · ✅ **위임 실행 완료 (2026-08-25 밤)**

> 관제탑 배정 검산 통과(음수 행 환입 규칙·#105 ⓐ·#117 안2 명시 승인) → 위임 실행.
> **사후검증 전부 기대 일치**(원시): 미귀속 14→**0** · 귀속 80→**94** · 등록별
> 21=+9 / 30=+5 / 31=+10 / 116=+3 / 117=−23 (19·20·104 = 귀속 0 유지) ·
> 학생 총진행 불변(김재성 157 · 장익교 56).
> **D구간 FIFO 백필 전량 종결 — 비시드 미귀속 0행.** 이하 발행 원문 보존.

> 선행 전부 해소: §7-6 정정 3건 실행 · #117 = 안2(현행 유지) 확정. 규칙 = §7-2(트레이너
> 일치 필수 · started_on FIFO · 기귀속 선차감) + 부분 초과 ⓐ(통째 — straddle (b) 동형).
> 학생 8·9는 기존 귀속 0이라 순수 FIFO. **규칙 보완 명시**: 음수(정정) 행은 소비가
> 아니라 환입이므로 「잔여>0」 조건 없이 **트레이너 일치 FIFO 첫 등록**에 귀속한다
> (두 학생 모두 해당 담당 등록이 실질 1축이라 배정이 갈리지 않는다).

#### 배정표 (14행 — 실측 played_at 순)

| 학생·담당 | 세션(판) | 귀속 등록 | 비고 |
|---|---|---|---|
| 김재성·준구(2) | #62(+4) · #90(−5) · #149(+10) | **#21**(3/19·33) | FIFO 첫 등록. #90은 #62의 오등록 정정 — 같은 등록에서 상쇄. 합 +9 |
| 김재성·현태(5) | #71(5) · #81(5) | **#31**(4/13·10) | 5+5 = 10 정확 소진 |
| 김재성·현태(5) | #110(5) | **#30**(4/23·33) | #31 소진 후 다음 FIFO |
| 장익교·준구(2) | #92(15) · #104(15) · #105(5) · #148(−32) | **#116**(5/07·33) | 준구 등록 유일. #105는 잔여 3에 5판 — **ⓐ 통째**. #148은 환입 |
| 장익교·현태(5) | #70(5) · #99(5) · #113(5) · #147(−38) | **#117**(7/15·33) | 현태 등록 유일(안2 — 담당 유지). #147은 환입 |

#### 발행 SQL (멱등 — null일 때만)

```sql
update public.lesson_sessions set lesson_enrollment_id = v.enr
  from (values (62,21),(90,21),(149,21),(71,31),(81,31),(110,30),
               (92,116),(104,116),(105,116),(148,116),
               (70,117),(99,117),(113,117),(147,117)) as v(id, enr)
 where public.lesson_sessions.id = v.id and public.lesson_sessions.lesson_enrollment_id is null;
-- 기대: UPDATE 14
```
...
```sql
-- ① 비시드 미귀속 14 → 0
select count(*) from lesson_sessions where lesson_enrollment_id is null and coalesce(created_by,'') <> 'seed';
-- ② 귀속 80 → 94
select count(*) from lesson_sessions where lesson_enrollment_id is not null;
-- ③ 등록별 귀속 합. 기대: 21=+9 · 31=+10 · 30=+5 · 116=+3 · 117=−23 (19·20·104는 0 유지)
select lesson_enrollment_id, sum(games) from lesson_sessions
 where lesson_enrollment_id in (19,20,21,30,31,104,116,117) group by 1 order by 1;
-- ④ 학생 총진행 불변(귀속만 변경): 김재성 157 · 장익교 56
```

⚠️ 등록별 표시 잔여(21=24·30=28 등)는 **개시잔액(시드) 미귀속 구조의 알려진 한계**로
담당 축 실잔여(§7-6 검산: 김재성 준구 0·현태 18 / 장익교 합 10)와 다르게 보인다 —
정본 잔여는 학생·담당 축 총량 대사(시드 포함)다. 등록별 잔여를 소비하는 화면이
생기면 시드 배분이 선행돼야 한다(현행 패널은 학생 축이라 무영향).
