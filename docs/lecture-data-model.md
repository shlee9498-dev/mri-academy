...
4개 축(디자인 계약 · 접근성 · 데이터 모델 정합 · 규칙 커버리지)으로 독립 검토 후
발견마다 반박을 시도했다. **65건 발견 · 57건 검증 완료(유지 34 · 반박기각 23)**.
아래는 그중 **2개 이상의 축에서 독립적으로 나온 것**만 추렸다 — 교차 확인이
단일 반박 시도보다 강한 신호다.

저장소 기계 검사(`.claude/skills/impeccable/scripts/detect.mjs`)는 **무출력·통과**다.
다만 그 검사는 범위가 좁다 — 고의로 심은 안티패턴 5개 중 1개만 잡았다.
아래는 전부 계약 문서를 읽어서 대조한 결과다.

### 10.1 🔴 데이터 모델과 정면 충돌 (착수 전 화면 수정 필요)

**① 슬롯당 차감 회차가 화면에 없다 — 허혜민 84,000원 사고의 재발 경로**

목업의 슬롯은 전부 3시간(03–06 · 07–10 · 10–13 · 14–17 · 20–23)이다.
§4.2에 따라 **같은 슬롯이 학생마다 다르게 차감된다**:

| 학생 | `session_minutes` | 3시간 슬롯 1건 | 활성 16명 중 |
|---|---|---|---|
| 신 체계(김해주·허혜민 신) | 180 | **1.0회** | 2명 |
| 구 체계(엄태현·신종근·이희훈 등) | 120 | **1.5회** | 다수 |

목업은 신 체계 학생(김해주) 기준이라 1슬롯=1회가 성립하지만,
구 체계 학생에게는 성립하지 않는다. **화면에 "이 예약은 1.5회 차감" 표기가 없으면
학생이 잔여를 오해하고, 그게 정확히 허혜민 84,000원이 발생한 경위다**(§4.3①).

→ 슬롯 카드에 차감 회차를 명시. 등록의 `session_minutes`로 학생마다 다르게 렌더.

**② '남은 회차 8'이 확정 예약 2건을 선점하지 않은 값이다**

상단에 `8`, 아래 내 예약 2건. §9.3의 가용회차 규칙대로면 **신청 가능한 건 6회**다.
한 숫자로 뭉개면 학생은 8회를 더 예약할 수 있다고 읽는다 — 잔여 우회의 입구다.

→ `남은 8 · 예약중 2 · **신청가능 6**` 3분할 표기. 미수 차단 UI의 핵심이다.

**③ 등록이 2건일 때 어느 등록으로 예약하는지 고를 수단이 없다**

가정이 아니다 — **허혜민이 실제로 구 체계 36회(종료)와 신 체계 8회를 동시에 보유**한다(§2.1).
차감 단가도 회차 환산도 다른 두 등록 중 어디서 빠지는지 화면이 말하지 않는다.

### 10.2 🔴 규칙 6종 중 화면에 대응 상태가 있는 건 1.5종뿐

| 규칙(§9.3) | 목업 상태 |
|---|---|
| 정원 초과 → 알림 전환 | ✅ `3/3 마감` + 🔔 |
| 시작 3시간 전 잠금 | 🔶 슬롯 1건만(`.locked`), 예약 측 취소 버튼은 단일 상태 |
| 잔여 0 → 불가 | ❌ **없음** (`.b-no` CSS만 있고 쓰이지 않는다) |
| 등급 미달 | ❌ **없음** — 심화 학생 기준이라 초급 학생 화면이 미정의 |
| 3시간 이내 취소 → 차감 | ❌ 경고·확인 UI 없음 |
| 무단 결석 → 차감+경고 | ❌ 없음 |

추가 누락: **지난 슬롯·진행 중 슬롯 상태**(목업 안에서 시간 모순 발생),
빈 상태 3종(슬롯 없는 날 · 예약 0건 · 잔여 0), 자정을 넘기는 슬롯의 날짜 귀속
(`03:00–06:00`이 이미 그 경계에 있다 — §1.3에서 `duration_min`을 저장하기로 한 이유와 같은 문제).

### 10.3 🔴 디자인 계약(DESIGN.md v2) 위반

| 계약 조항 | 목업 |
|---|---|
| `--gold:#f5c518` = **유일한 액센트** | 화면에 **0회** 등장 |
| **"보라·파랑 액센트 금지"** · AI 티 5대 패턴 #2 | 아바타 보라 그라데이션 `#2B1F45→#3D2A63`, 뱃지·`lv3`·`.b-bell` 보라, 주 CTA `.b-go:#2C5FD4` 파랑 |
| `--bg:#0a0a0c` (중립 다크) | `#080B14` (네이비 톤) — 표면·선 토큰 전부 불일치 |
| **"글래스/blur 미채택(성능)"** | `backdrop-filter:blur(14px)` 2곳(`.top`·`.tabs`) |
| 디스플레이 폰트(영문·숫자) | 미로드 — 전부 Pretendard |
| 본문 최소 16px·행간 1.6+ | 최대 본문 15.5px, 행간 선언 없음 |

**절충안 제안**: 시간대 5색을 통째로 버릴 필요는 없다.
DESIGN.md에 이미 *"티어 엠블럼 원색은 '인용된 콘텐츠'로만 허용(UI 색으로 승격 금지)"* 라는
예외 논리가 있다. 시간대 색에 그대로 적용하면:

- **시간대 색 = 좌측 3px 바에만 한정**(인용된 콘텐츠 취급) → 시간표 이미지와의 학습 전이 유지
- **UI 강조(버튼·뱃지·활성 탭·선택 날짜·아바타) = 골드 단일** → 계약 준수
- 시간대는 이미 이모지+레이블(🌑 새벽 등)로 중복 표시돼 있어 색이 유일 신호가 아니다

이건 오너 결정 사항이다 — "시간표 이미지와 동일 팔레트"라는 의도와 계약이 충돌하므로.

### 10.4 접근성 (전부 blocker 판정)

- **`.slot.locked{opacity:.42}`가 잠긴 슬롯 전체를 대비 1.35~3.63:1로 붕괴시킨다.**
  `opacity`는 그룹을 먼저 렌더한 뒤 배경에 합성하므로 안의 모든 텍스트가 실제로는
  합성색으로 보인다. 실측: `.stime` 2.06:1 · `.seats .l` 1.52:1 · `.b-lock` 라벨 **1.35:1**.
  그 안에 담긴 건 장식이 아니라 *왜 신청할 수 없는지* 판단할 실데이터(시간·등급·자리)다.
- **`--faint:#5B6B8F` = 3.69:1** — DESIGN.md가 *"본문 사용 금지"* 로 못 박은
  `#71717a`(4.09:1)**보다도 더 어둡다.** 11.5px `.note`에 쓰이고 있다.
- **터치 타겟** — 주 CTA ≈36px, `취소` ≈26px (기준 44×44px)
- **`.day`·`.tab`·`취소`가 `div`/`span`** → 키보드로 도달 불가. 페이지 전체에 `:focus` 스타일 0개
- 날짜 `pip`이 색만으로 가용 여부를 구분(계약: *"상태를 색에만 의존해 구분하지 않는다"*)

### 10.5 표기 문제

- **`N/3`의 분자가 예약수인지 남은 자리인지 라벨과 충돌**한다. `0/3`인데 `자리`라고 적혀 있어
  "남은 자리 0"으로 읽히는데 실제로는 신청 가능이다.
  → `2자리 남음` 형태 권고. (커밋 `1dd5f6e` **"좌석 표기 직관화"** 에서 이미 한 번 겪은 함정이다)
- **`마감`이 두 상태에 중복** — 정원 마감(`3/3`)과 시간 잠금(`🔒 마감`).
  잠금은 `🔒 신청 마감됨`이 아니라 `🔒 03:00 잠김` 같은 사유 표기가 맞다.
- 알림 토스트에 **10분 우선권 잔여 시간이 없다.** 우선권이 UX 핵심이라면 남은 시간이 보여야 한다.

### 10.6 목업 파일 자체

실존 수강생의 실제 계약 정보(실명·반·회차)가 하드코딩돼 있다.
저장소에 넣을 때는 더미 데이터로 바꾸는 게 맞다 — 문서 내 실명 인용과 달리
목업은 **Vercel 프리뷰로 공개 URL이 생기는** 정적 페이지다.

### 10.7 일정 영향

§9.6의 `6. booking.html 구현 3일`은 **위 수정을 포함하면 4~5일**로 본다.
상태 정의가 5종에서 12종 이상으로 늘고(잔여0·등급미달·지난·진행중·잠금취소·빈상태 3종 추가),
등록 선택 UI와 차감 회차 표기가 새로 붙는다.

→ **MVP 총계 16일 → 17~18일.** 8/31이 이미 여유 없던 만큼,
