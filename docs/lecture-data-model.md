...
### 8.3 동명 6쌍 — 판정 확정 (2026-08-14 오너 회신)
...
오너 전수 판정(8/14) 결과 — SQL은 채팅 전달(Level 0), 실행 후 기대 행수 대조:

| 쌍 | 판정 | 조치 |
|---|---|---|
| 김재성 8/15 | 동일인·병행 | **병합 15→8**(생존 8·준구 — 잔여 보유 담당이 패널에서 봐야 함. 현태 이력은 등록 trainer 5로 보존) |
| 염창덕 36/44 | 동일인 | **병합 44→36**(생존 36·현태) + `status='done'` |
| 윤다온 13/21 | 동일인 | **병합 21→13**(생존 13·현태) + `status='paused'`(재시작 예정) |
| 이강준 42/50 | **병합 판정 보류** | 둘 다 `status='done'`(환불 130,900 완료·양쪽 마감 — 오너 8/14). 백필 대사 시 보류 표기 |
| 이대호 29/53 | **보류·무조치** | 오너 지시(제외). 5/25↔5/27 미러형 — 매출 120,000 과대 가능성 기록만 |
| 최재민 27/52 | **보류·무조치** | 오너 지시(중단 인원). 결제 0건이라 금전 영향 없음 |

부수 판정(같은 회신): 이희훈(id14) = `paused` + 5/14 결제 2건 유지(원장 무근거이나 오너 판정
— 환불 불요, **급여 귀속은 무리로 이동**) · 김대윤 판수 54→21(원장 r147) + 8/1 99판 신규 ·
심재원 4/10 2건 = 둘 다 실결제(원장 r33·r34 "10판 1/2회차") 무조치.
...
## 9. 강의 병행 케이스 — 강의는 `courses`에 붙는다 (2026-08-15 실측)

레슨은 현태, 강의는 무리(직강)인 **동형 2건**이 실측으로 확인됐다.

| 학생 | `students.trainer_id` | 레슨 | 강의(미귀속) |
|---|---|---|---|
| 허혜민 (57) | 5 현태 | 3건 · 등록 91·92·93 | `payments` #137 · 8/2 · 416,000 |
| 김해주 (10) | 5 현태 | 1건 · 등록 22 | `payments` #142 · 8/1 · 290,000 |

둘 다 memo에 `담당 무리`가 있고 `course_id`는 null이다. **`courses` 테이블은 0행**(P1 미착수)이라
강의 축이 아직 비어 있다.

### 9.1 강의용 등록을 `lesson_enrollments`에 만들면 안 된다

병행을 표현하려고 강의분을 레슨 등록으로 만드는 안이 나올 수 있는데, 세 가지가 막는다:

1. **CHECK가 막는다** — `chk_payments_single_attribution`은 `num_nonnulls(course_id, lesson_enrollment_id) <= 1`이다.
   설계가 두 축을 명시적으로 분리해 뒀고, 강의 결제를 레슨 등록에 붙이면 그 의도를 우회하는 것이 된다.
2. **단위가 다르다** — `lesson_enrollments.games_total`은 **판수**고 강의는 **회차(units)** 다.
   심화 8회를 `games_total=8`로 넣으면 판수 정본(계약총 2,883판)에 8판이 섞인다. 잔여·차감·정산이
   전부 판수 기준이라 조용히 오염된다.
3. **강의 전용 필드가 `courses`에만 있다** — `level`·`scheme`·`session_minutes`·`unit_price`·`units_total`.
   강의 정산 근거가 이 필드들이라 레슨 등록에는 담을 자리가 없다.

### 9.2 막고 있는 것 — `courses`에 담당 컬럼이 없다

"레슨(현태)·강의(무리)가 각자 화면에 뜬다"를 성립시키려면 강의의 담당이 DB에 있어야 하는데,
`courses`에는 `trainer_id`가 **없다**. 지금은 `payments.memo`의 `담당 무리` 문자열이 유일한 근거다 —
`docs/lesson-enrollment-model.md` §5.1이 지적한 "담당이 memo에만 있다"와 정확히 같은 결함이다.

**제안(P1 착수 시 함께):** `courses.trainer_id bigint references staff(id)` 신설(Level 0 DDL) →
...
### 9.4 결제액 ≠ 계약액은 정상 케이스다 (오너 판정 2026-08-16)

`payments` 1행이 강의 계약액과 다른 금액일 수 있다. 이건 오류가 아니라 **정상 상태**이며,
등호를 전제하는 검산·가드를 넣으면 안 된다.

**판례 — #137 허혜민 8/2 416,000**

| 항목 | 금액 |
|---|---|
| 결제 1행(#137) | **416,000** |
| 강의 계약 정가(심화 8회 × 36,250) | 290,000 |
| 차액 = 구 체계 잔여분 정산 | 126,000 |

**판정: 단일 행 유지 + 전액 course 귀속.** 근거 두 가지 —

1. **통장 1줄이 대사의 단위다.** 통장에 한 줄로 들어온 입금을 화면 편의로 쪼개면 원장과 통장이
   어긋나고, 그 순간 대사(reconciliation)의 기준점이 사라진다.
   ⚠️ **이 원칙의 표현은 2026-08-16 세트 판매 확정으로 개정됐다** — 「통장 1줄 = `payments` 1행」이
   아니라 **「통장 1줄 = `deposit_ref` 1개」**다(§9.5). #137은 귀속처가 하나뿐이라 여전히 1행이고
   `deposit_ref`는 null이다(`coalesce`가 `'P'||id`로 대체하므로 대조에서 자기 자신이 한 묶음이다).
   **판정은 그대로 유지된다** — 바뀐 것은 "쪼개면 안 된다"가 아니라 "쪼갤 때 무엇으로 다시 묶느냐"다.
2. **§19 `chk_payments_single_attribution`.** 결제 1건의 귀속처는 하나다. 한 행을 두 귀속처에
   걸 수 없으므로, 분할하지 않는 이상 전액이 한쪽으로 간다.

**`courses` 행은 계약 정가로 만든다** — `unit_price=36250` · `units_total=8`(정가 290,000).
결제액을 역산해 계약을 맞추지 않는다. 계약은 계약이고 입금은 입금이다. 차액의 정체는
`payments.memo`에 남긴다(`구 체계 잔여분 정산(강의 계약액과 별개)`).

**그래서 P1에서 지켜야 할 것** — 강의 잔여·소진 계산의 분모는 **언제나 `units_total`**이고
결제액이 아니다. `sum(payments.amount) = unit_price × units_total`을 검산식으로 쓰면 이 건이
즉시 오탐으로 뜬다. 금액 대조가 필요하면 `>=`(미납 탐지)로 쓰고 `=`로 쓰지 않는다.

**재판정 조건(오너 단서)** — 126,000이 **레슨 판수 구매분**으로 확인되면 분할을 재판정한다.
2026-08-16 실측으로는 **해당 없음**:

- 허혜민 레슨 결제 3건(40,000/10판 · 120,000/33판 · 240,000/66판)은 **전부 등록 93·92·91에
  귀속 완료**이고 판수도 일치한다 — 미귀속 레슨 결제가 없다.
- 그의 실제 판당 단가는 4,000 / 3,636원인데 126,000은 어느 쪽으로도 정수 판수가 안 나온다
  (31.5판 / 34.65판).

따라서 현재 근거로는 **미납 정산분**이 맞고, 위 판정을 그대로 둔다.

---

### 9.5 세트 판매 = `payments` 2행 (오너 확정 2026-08-16 · 구현 2026-08-17)

세트(강의 + 레슨 묶음 상품) 1건은 **결제 2행**이다. 화면 편의가 아니라 §19d
`chk_payments_single_attribution` 때문이다 — 한 결제 행의 귀속처는 `course_id`·
`lesson_enrollment_id` 중 **최대 하나**다. 세트는 양쪽에 붙어야 하므로 행을 나누는 것 외에
방법이 없다. 두 행은 같은 `deposit_ref`로 묶여 통장 1줄에 대응한다.

`parent_payment_id`(부모-자식) 방식은 기각됐다 — 부모 결정 규칙과 삭제 순서가 꼬인다.
`deposit_ref`는 대등한 형제 묶음이라 그 문제가 없다.

#### 행 구조

| | `kind` | `amount` | `games` | 귀속 | 비고 |
|---|---|---|---|---|---|
| **강의행** | `set` | 정가 − **할인 전액** | `0` | `course_id` | 판수 결제가 아니다 |
| **레슨행** | `set` | **단품 정가 유지** | 판수 | `lesson_enrollment_id` | 단가 스냅샷의 분자 |

예: 입문세트 280,000 = 강의행 235,000 + 레슨행 45,000.

**할인을 강의행이 전액 흡수하는 이유** — 레슨행 금액이 그대로 `lesson_enrollments.paid_amount`
(§19b-1 단가 스냅샷)가 되고, 그 값이 환불 산식의 분자다. 레슨행에 할인을 나눠 실으면 환불이
할인율만큼 과소지급된다. `courses.unit_price`는 계약 정가를 유지하고 차액은 `memo`에 남긴다
(#137과 같은 패턴 · §9.4).

#### `deposit_ref` 명명 규칙

`'D' || 입금일(YYYYMMDD) || '-' || 그날의 2자리 순번` → `D20260817-01`.
**키의 단위는 통장 1줄**이다. 사람 이름을 넣지 않는다 — PII이고 동명이 실재한다(§8.3).
단일 결제는 `null`로 두고 대조에서 `coalesce(deposit_ref, 'P'||id)`로 대체하므로 접두어
`D`/`P`가 갈려 두 계열이 섞이지 않고, **기존 130행 마이그레이션이 불필요**하다.

#### 생성 SQL (멱등 · Level 0 — 오너 실행)

```sql
-- 세트 1건 = 2행. <> 안의 값만 채운다. 재실행해도 같은 deposit_ref면 0행 삽입.
insert into public.payments
  (student_id, paid_at, amount, games, kind, payout_rate, pay_channel, deposit_ref,
   course_id, lesson_enrollment_id, memo, source)
select * from (values
  -- 강의행: 할인 전액 흡수 · games 0 · 귀속 = course_id
  (<학생id>, '<입금일>'::date, <강의행금액>, 0, 'set', <지급율>, '<채널>', '<D20260817-01>',
   <course_id>, null::bigint, '세트 <상품명> 강의분', 'manual'),
  -- 레슨행: 단품 정가 · 귀속 = lesson_enrollment_id
  (<학생id>, '<입금일>'::date, <레슨행금액>, <판수>, 'set', <지급율>, '<채널>', '<D20260817-01>',
   null::bigint, <등록id>, '세트 <상품명> 레슨분', 'manual')
) as v(student_id, paid_at, amount, games, kind, payout_rate, pay_channel, deposit_ref,
       course_id, lesson_enrollment_id, memo, source)
where not exists (select 1 from public.payments p where p.deposit_ref = v.deposit_ref);
```

**선행 조건** — `courses` 행과 `lesson_enrollments` 행이 **먼저** 있어야 한다(귀속 대상).
스키마 변경이 없으므로 `notify pgrst`는 불요다. 실행 후 검산은
`lesson-enrollment-model.md` §8.2의 묶음 쿼리 3종.

**수수료 컬럼은 템플릿에서 뺐다** — `fee_amount`는 `not null default 0`, `net_amount`는 nullable이라
생략해도 삽입은 통과한다(실DB 확인 2026-08-17). 계좌이체(`transfer`)는 수수료가 0이라 그대로 두면
맞고, **`groble`이면 두 행에 각각 채워야 한다**(패널 경로의 `feeFor`/`netFor`에 해당). 이때 행별로
반올림되므로 묶음 `net` 합이 통장 순입금과 ±1원 어긋날 수 있다 — 아래 ③ 참조.

#### 코드 영향 — 실측 (2026-08-17 · `admin-panel.js` 실마운트 하네스)

**① 정산 엔진은 2행 구조를 이미 올바로 읽는다 — 수정 불요.** `computeStudent`의 판수 결제
필터가 `["lesson","set"] && games > 0`이라 레슨행만 들어오고 강의행(`games=0`)은 자동으로 빠진다.

| 입력 형태 | 단가 | 지급예정(진행 4판·요율 0.65) |
|---|---|---|
| **2행**(레슨행 45,000/10판) | 4,500 | **11,700** ✅ |
| 1행(280,000/10판) | 28,000 | **72,800** ❌ |

**② 그래서 1행 세트는 차단한다(구현).** 1행으로 넣으면 강의분까지 레슨 결제로 세어 지급예정이
**6.22배(+61,100원)**가 되는데, 화면에는 "세트를 등록했다"로만 보여 틀린 줄 모른다.
`POST /api/admin/payments`는 `kind='set'`을 400(`set_needs_two_rows`)으로 돌려보내고 위 SQL로
안내한다. 기존 호출자는 `kind`를 보내지 않으므로(`staff-panel.html` `addPayment()`) 영향 0.

**③ 대조는 `amount`(총액)로 한다.** `net_amount` 합은 채널 수수료가 행마다 반올림돼
묶음 합이 통장 순입금과 어긋날 수 있다 — 실측 최대 **±1원**(groble 4.84% · 10만~60만 전 구간
스윕). 세트 3종 조합(280/380/480천)에서는 편차 0이지만, 등호 검산은 `amount`로만 세운다.

**④ ⚠ 결제 트랙 판정 필요 — 세트는 빵다 순매출 6% 대상에서 통째로 빠진다.**
`computeStaffSalary`가 2026-08부터 `kind === 'lesson'`만 base로 잡는다(`LESSON_ONLY_START`).
세트의 레슨행은 `kind='set'`이라 제외된다 — 실측: 560,000 세트 → 6% 제안 **0원**, 같은 금액이
`lesson`이었다면 30,500원. `kind='set'` 한정은 강의·상담이 6% 대상으로 자동 확대되는 것을 막으려던
필터인데 세트의 **레슨분**까지 함께 걷어낸다. 의도 여부는 급여 산식이라 **결제 트랙 소관**이고,
이 트랙은 판정만 요청한다(코드 미변경).

#### DB가 막지 못하는 것

"한 묶음은 정확히 2행이고 귀속이 서로 다르다"는 **행 간(cross-row) 조건**이라 CHECK의 표현
범위 밖이다(CHECK는 행 단위). 트리거는 결제 트랙 소관이라 만들지 않는다. 따라서 **반쪽 묶음·
귀속 중복은 상시 검산 쿼리로만 잡힌다** — `lesson-enrollment-model.md` §8.2.
