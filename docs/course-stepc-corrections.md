# STEP C — 강의(courses) 축 정정 2건 · 발행 (2026-08-25 밤 · 관제탑 7회차 인라인 수신)

> **발행만 — 실행은 관제탑 검토 → 위임 승인 후** (관제탑 지시 순서 그대로).
...
- **박성민 미수**: 완료 15회 = 계약 12회 대비 −3회(−90,000), 미기입 5건 반영 시
  −8회(−240,000) — **오너 판정 대기**(`settlement-corrections.md` 대기 목록 등재).
- 이 발행 실행 시 `REQUIRED_SCHEMA`·코드 변경 없음(기존 컬럼·CHECK 전부 통과 실측).
