...
「v1.1 학생 매핑 승인 큐」·「PR-6(/my 포털)」을 아래에서 검색했으나 나오지 않았다:

- `docs/` 전체 · 루트 `*.md` 전체
- 전 원격 브랜치 (`git grep` 전 ref)
- 커밋 메시지 (`--grep`)

**저장소 밖 문서로 보인다**(지휘탑 채팅 또는 MRIacademy 통합 전환 트랙 세션).
따라서 아래 대조는 **저장소 안의 정본(`docs/lecture-data-model.md`)** 과만 한 것이다.

### 6.2 겹치는 지점

| 항목 | 이 문서 | 저장소 정본 | 충돌 시 |
|---|---|---|---|
| 학생 신원 연결 | §2.3-C에서 **설계하지 않고 위임** | `lecture-data-model.md:742` 오너 승인 큐 | **v1.1 우선** |
| `/my` 포털 | §5.2에서 Phase 2로만 표기 | `:772` 「학생 인증(OAuth 분기 + 승인 큐) 2일」 | **v1.1(PR-6) 우선** |
