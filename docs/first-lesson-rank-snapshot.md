# 첫 수업 시점 경쟁전 스냅샷 — `student_rank_snapshots` (§30a 초안 · 2026-09-26 · 오너 지시 · §29 와 분리)

> 상태: **설계 초안 · 코드 없음 · DDL 미실행.** 인증 후기(반장 `mri-student-app/docs/testimonial-design.md` v0.2 · #26 · §3a)의 「수강 전후 경쟁전 점수」 비교용으로, 후기보다 먼저 쌓기 시작한다.
> 구현 순서(오너 9/26): **§29 운영 실행 뒤** → 이 DDL(오너 · 「최종」 재발행) → 서버 Draft PR. 번호는 §29 와 별도 — **§30 = 인증 후기 묶음(반장 「§30 후보」) · 30a = 이 표(먼저) · 30b~ = 후기 본체(후기 문서 확정 뒤)** 로 제안한다(오너가 다른 번호를 주면 그대로).
...
| 전제 | `pubg_name` 보유 수강생이 적다(2026-09-25: active·paused 13/74 · 최근 30일 첫 수업 11명 중 0명) → §28 닉네임 설계가 있어야 실제로 쌓인다. 없으면 행은 pending 으로 7일 재시도 뒤 `given_up`(닉네임이 7일 안에 들어오면 그때 잡힌다 · §5 ①) |

## 1. 요구 ↔ 반장 후기 v0.2 §3a 대조

| 항목 | 반장 §3a | 오너 9/26 | 이 문서 |
|---|---|---|---|
...
| 트리거 | 첫 `lesson_sessions` 등록 **또는 첫 강의 출석** | 「첫 수업이 기록될 때」(`/수업등록`) | 1차 = `/수업등록`(레슨) · 강의 첫 출석(`course_attendance`)은 오너 판정 뒤(§5 ③) |
| 실패 | 항목 제외 | 하루 1회 재시도 · 7일 뒤 포기 · +n일 표시 | `status`·`attempts`·`next_try_at`·`days_after` |

## 2. DDL 초안 — §30a (오너 실행 · STATE 머리말 형식 · 「최종」은 §29 실행 뒤 재발행)
...
4. **표시**: 후기·성장 화면은 `days_after > 0` 이면 「첫 수업 +n일」 을 붙인다(값 자체는 그대로). 1차 서버 API 노출은 없음(후기 문서 확정 뒤 · 필요하면 트레이너 포털 `GET /students[]` 에 `firstLessonRank` 요약).
...
표 1 · 기존 표 변경 0 · PUBG 호출 첫 수업당 ≤2(+재시도 일 1회) · 서버 PR 1개(훅 1 · 크론 1 · 자기점검 1행 · ~150줄) · 반나절 · 구현은 §29 실행 뒤.

## 5. 오너 판정 (구현 전)

| # | 질문 | 이 문서 기본값 |
|---|---|---|
| ① | `pubg_name` 이 없을 때 행을 만들지 | **만든다**(pending · `no_pubg_name` · 7일 재시도 → 그 안에 닉네임이 들어오면 「+n일」 로 잡힘). 오너 문장대로 「있을 때만」 이면 한 줄 변경 |
| ② | § 번호 | **§30a**(§30 = 후기 묶음 · 반장 후보 번호 유지) |
| ③ | 강의(직강) 첫 출석도 「첫 수업」 인지 | 1차 = 레슨 `/수업등록` 만 · 강의는 `course_attendance` 경로 확정 뒤 |
| ④ | 값 표시 위치 | 후기 문서 확정 뒤(1차 API 노출 없음) |
