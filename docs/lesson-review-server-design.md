...
**이관 설계 근거 (2026-09-26 실측 · 읽기 전용)** — 미승인 59건 전부(`published` 0 · `rejected` 0 · 최장 94일)에서 개인정보 패턴 적중 **0건**이다(전화번호 · 계좌형 `xxx-xx-xxxx` · 10자리 이상 연속숫자 · 은행명 · 수강생 실명 모두 0. 1차 스캔의 20건은 「우리」·「하나」 오탐이었다). 공개 경로 `GET /api/feedback-public` 은 `students.name`·`discord_nick` 사전으로 본문 실명을 「레슨생」으로 런타임 치환한다(`server.js` §공개 코칭 기록). 따라서 이관 때 마스킹을 새로 설계할 필요가 없고, 원문 재수집도 개인정보 선별 없이 진행할 수 있다. 같은 날 오너 판정으로 미승인 적체 봇 알림(`fbPending`)은 중단했다 — 이 표는 이관에서 한 방향으로만 만진다.
...
19. **PR-3 구현에서 정한 것(계약 = `docs/trainer-portal-api.md` §8.9)**: 트레이너 목록 = 받는 트레이너가 나 ∪ `scopedStudents` 학생의 **보낸** 복기(초안은 트레이너에게 없다 · 트레이너 작성 초안은 2차) · `days` 는 **보낸 시각** 기준(기본 30 · 1~365) · `status` 는 `published` 만(그 밖 400) · 오너 목록도 같은 범위(오너의 넓은 권한은 상세·답·지우기에만) · `unread` = 읽은 기록 없음 ∨ 읽은 뒤 수정(`updated_at`) · `awaitingReply` = 받는 트레이너 ∧ 내 comment·overall 0건(반응 제외) · 답 권한 없음도 404(403 없음 원칙 그대로 · 앱은 `canReply` 로 가린다) · 답 수정은 **본문만 · 내 답만**(오너도 남의 답 수정 불가) · 지우기는 내 답 + 오너 · 1차 kind = comment·overall 만(mark·task 는 켜기 전까지 400 — `FEEDBACK_KINDS_ON`) · 숨김은 오너 포함 모두 404(§5.5 — 오너 확인은 SQL · §4 의 「오너만 hidden:true」 는 쓰지 않는다) · 트레이너 화면 작성자 = `authorDisplayName`(이름) + `authorPubgName` · 그 복기의 수강생 = `studentDisplayName`·`studentPubgName`(예약 대조용) · 트레이너 상세는 반응자 목록을 모든 트레이너에게 · 공개 열람 트레이너는 원본 URL·세션 id·받는 사람 없음.
...
| 6 | **PR-3(2026-09-25 · 계약 §8.9 · 통합 시험 +82 = 295 · 단위 +3 = 44)** 트레이너 포털(목록·상세·comment/overall·읽음·`canReply`·`replyDueAt` 자리 · **feed·reactions(한 번 탭)·공유 열람(활성 트레이너 전원)** · task 는 2차지만 `due_invalid` 검사 함수(`dueCheck`)는 여기서) + `docs/trainer-portal-api.md` §8.9 계약 · 코드 = `review-api.cjs` `mountTrainer`(trainer-portal 뒤 · 게이트·`requireTrainer`·`scopedStudents`·scrubTrainer 공유) | 이 세션 | DDL 없음(§29 그대로) · env 없음 |
